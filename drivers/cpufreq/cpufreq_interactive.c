...
static ssize_t show_max_freq_hysteresis(
	struct cpufreq_interactive_tunables *tunables, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", tunables->max_freq_hysteresis);
}
static ssize_t store_max_freq_hysteresis(
		struct cpufreq_interactive_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	...
	if (ret < 0)
		return ret;
	tunables->max_freq_hysteresis = val;
	return count;
}
		...
		unsigned int code, int value)
{
	...
	for_each_online_cpu(i) {
		...
		tunables->boostpulse_endtime = ktime_to_us(ktime_get()) + TOUCHBOOST_DURATION;
		cpufreq_interactive_boost(tunables);
	}
}
...
static int __init cpufreq_interactive_init(void)
{
	...
	i = input_register_handler(&interactive_input_handler); /* i is unused */
